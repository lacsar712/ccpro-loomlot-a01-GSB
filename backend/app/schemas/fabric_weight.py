...
class FabricWeightCreate(BaseModel):
...
class FabricWeightUpdate(BaseModel):
    dye_lot_id: Optional[int] = Field(None, alias="dyeLotId")
    weighed_at: Optional[datetime] = Field(None, alias="weighedAt")
    weight_kg: Optional[float] = Field(None, gt=0, alias="weightKg")
    recorder_name: Optional[str] = Field(None, min_length=1, max_length=64, alias="recorderName")

    model_config = ConfigDict(populate_by_name=True)


class FabricWeightOut(BaseModel):
